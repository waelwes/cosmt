...
export default function PTTSettingsForm() {
  const [config, setConfig] = useState<PTTConfig>({
    // API Credentials
    apiKey: '',
    customerId: '',
    apiEndpoint: 'https://api.ptt.gov.tr',
    mode: 'sandbox',
  ...
  const loadSettings = async () => {
    try {
      const response = await fetch('/api/shipping/settings?provider=ptt');
      if (response.ok) {
      ...
      console.error('Error loading PTT settings:', error);
    }
  };

  const handleInputChange = (field: keyof PTTConfig, value: any) => {
    setConfig(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const validateConfiguration = async () => {
    ...
    try {
      ...
      if (result.valid) {
        toast.success('PTT Kargo configuration is valid!');
      } else {
        toast.error('PTT Kargo configuration validation failed');
      }
    } catch (error) {
      setValidationResult({
        valid: false,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('PTT Kargo settings saved successfully!');
        setValidationResult(null);
      } else {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Input
              ...
              placeholder="Enter PTT Kargo API key"
            />
          </div>
          <div>
            ...
            <Input
              ...
              placeholder="Enter PTT Kargo customer ID"
            />
          </div>
          <div>
            ...
            <Select value={config.apiEndpoint} onValueChange={(value) => handleInputChange('apiEndpoint', value)}>
              ...
              <SelectContent>
                <SelectItem value="https://api.ptt.gov.tr">Production</SelectItem>
                <SelectItem value="https://api-test.ptt.gov.tr">Test</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="enabled">Enable PTT Kargo</Label>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Select value={config.defaultService} onValueChange={(value) => handleInputChange('defaultService', value)}>
              ...
              <SelectContent>
                <SelectItem value="PTT_STANDART">PTT Kargo Standart</SelectItem>
                <SelectItem value="PTT_EXPRESS">PTT Kargo Express</SelectItem>
                <SelectItem value="PTT_ECONOMIC">PTT Kargo Ekonomik</SelectItem>
                <SelectItem value="PTT_SECURE">PTT Kargo Güvenli</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
