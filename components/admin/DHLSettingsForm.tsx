...
export default function DHLSettingsForm() {
  const [config, setConfig] = useState<DHLConfig>({
    ...
    // Shipping Options
    defaultService: 'DHL_EXPRESS_WORLDWIDE',
    labelFormat: 'PDF',
    weightUnit: 'kg',
    dimensionUnit: 'cm',
    defaultWeight: 1.0,
  ...
  const loadSettings = async () => {
    try {
      const response = await fetch('/api/shipping/settings?provider=dhl');
      if (response.ok) {
  ...
  const handleInputChange = (field: keyof DHLConfig, value: any) => {
    setConfig(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const validateConfiguration = async () => {
    ...
    try {
      ...
      if (result.valid) {
        toast.success('DHL configuration is valid!');
      } else {
        toast.error('DHL configuration validation failed');
      }
    } catch (error) {
      setValidationResult({
        valid: false,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Input
              ...
              placeholder="Enter DHL API username"
            />
          </div>
          <div>
            ...
            <Input
              ...
              placeholder="Enter DHL API password"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <Select value={config.apiEndpoint} onValueChange={(value) => handleInputChange('apiEndpoint', value)}>
              ...
              <SelectContent>
                <SelectItem value="https://api-eu.dhl.com">Europe (EU)</SelectItem>
                <SelectItem value="https://api.dhl.com">Global</SelectItem>
                <SelectItem value="https://api-sandbox.dhl.com">Sandbox</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="enabled">Enable DHL Shipping</Label>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Select value={config.defaultService} onValueChange={(value) => handleInputChange('defaultService', value)}>
              ...
              <SelectContent>
                <SelectItem value="DHL_EXPRESS_WORLDWIDE">DHL Express Worldwide</SelectItem>
                <SelectItem value="DHL_EXPRESS_12">DHL Express 12:00</SelectItem>
                <SelectItem value="DHL_EXPRESS_10">DHL Express 10:30</SelectItem>
                <SelectItem value="DHL_EXPRESS_9">DHL Express 9:00</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
            <Label htmlFor="labelFormat">Label Format</Label>
            <Select value={config.labelFormat} onValueChange={(value) => handleInputChange('labelFormat', value)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      {/* Webhook & Real-time Settings */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Webhook & Real-time Settings</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Input
              ...
              placeholder="https://yourdomain.com/api/webhooks/dhl"
            />
          </div>
          <div className="flex items-center space-x-2">
            <Switch
              id="realtimeRate"
              checked={config.realtimeRate}
              onCheckedChange={(checked) => handleInputChange('realtimeRate', checked)}
            />
            <Label htmlFor="realtimeRate">Enable Real-time Rate Calculation</Label>
          </div>
        </div>
      </div>
